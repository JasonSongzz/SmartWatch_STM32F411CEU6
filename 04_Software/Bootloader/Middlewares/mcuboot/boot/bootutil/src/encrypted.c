...
boot_decrypt_key(const uint8_t *buf, uint8_t *enckey)
{
    ...
    if (rc) {
        BOOT_LOG_ERR("ECIES private-key retrieval failed rc=%d", rc);
        return rc;
    }

    if (bootutil_enc_key == NULL || bootutil_enc_key->key == NULL ||
        bootutil_enc_key->len == NULL || *bootutil_enc_key->len == 0U) {
        BOOT_LOG_ERR("ECIES private-key descriptor is invalid");
        return -1;
    }
    BOOT_LOG_DBG("ECIES private-key descriptor loaded DER_len=%u",
                 (unsigned int)*bootutil_enc_key->len);
    ...
    if (rc) {
        BOOT_LOG_ERR("ECIES-P256 PKCS8 private-key parse failed rc=%d", rc);
        return rc;
    }
    BOOT_LOG_DBG("ECIES-P256 private scalar parsed");
    ...
    if (rc != 0) {
        BOOT_LOG_ERR("ECIES-P256 ECDH shared-secret calculation failed rc=%d", rc);
        return -1;
    }
    BOOT_LOG_DBG("ECIES-P256 ECDH shared secret calculated");
    ...
    if (rc != 0 || len != (BOOT_ENC_KEY_SIZE + BOOTUTIL_CRYPTO_SHA256_DIGEST_SIZE)) {
        BOOT_LOG_ERR("ECIES HKDF-SHA256 failed rc=%d len=%u", rc, (unsigned int)len);
        return -1;
    }
    BOOT_LOG_DBG("ECIES HKDF-SHA256 derived AES/HMAC key material");
    ...
    if (bootutil_constant_time_compare(tag, &buf[EC_TAG_INDEX], EC_TAG_LEN) != 0) {
        BOOT_LOG_ERR("ECIES HMAC-SHA256 authentication tag mismatch");
        return -1;
    }
    BOOT_LOG_DBG("ECIES HMAC-SHA256 authentication tag verified");
    ...
    if (rc != 0) {
        bootutil_aes_ctr_drop(&aes_ctr);
        BOOT_LOG_ERR("ECIES wrapped AES key decryption failed rc=%d", rc);
        return -1;
    }

    bootutil_aes_ctr_drop(&aes_ctr);

    BOOT_LOG_INF("ECIES-P256 encrypted image key authenticated and decrypted");
    rc = 0;
              ...
              struct boot_status *bs)
{
    ...
    if (rc) {
        BOOT_LOG_ERR("encrypted-key TLV iterator initialization failed slot=%d rc=%d", slot, rc);
        return -1;
    }

    rc = bootutil_tlv_iter_next(&it, &off, &len, NULL);
    if (rc != 0) {
        BOOT_LOG_ERR("ENC_EC256 TLV not found slot=%d rc=%d", slot, rc);
        return rc;
    }

    BOOT_LOG_DBG("ENC_EC256 TLV found slot=%d off=%" PRIu32 " len=%u",
                 slot, off, (unsigned int)len);
    if (len != BOOT_ENC_TLV_SIZE) {
        BOOT_LOG_ERR("ENC_EC256 TLV length invalid got=%u expected=%u",
                     (unsigned int)len, (unsigned int)BOOT_ENC_TLV_SIZE);
        return -1;
    }
    ...
    if (rc) {
        BOOT_LOG_ERR("ENC_EC256 TLV flash read failed slot=%d rc=%d", slot, rc);
        return -1;
    }

    rc = boot_decrypt_key(buf, bs->enckey[slot]);
    if (rc != 0) {
        BOOT_LOG_ERR("encrypted image key unwrap failed slot=%d rc=%d", slot, rc);
    } else {
        BOOT_LOG_INF("encrypted image key loaded slot=%d", slot);
    }
    return rc;
}
