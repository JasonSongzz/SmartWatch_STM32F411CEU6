                     ...
                     )
{
    ...
    if (rc) {
        BOOT_LOG_ERR("image hash calculation failed area=%u rc=%d",
                     flash_area_get_id(fap), rc);
        goto out;
    }
    BOOT_LOG_DBG("image SHA-256 calculated area=%u", flash_area_get_id(fap));
    ...
    while (true) {
        ...
        case EXPECTED_HASH_TLV:
        {
            ...
            if (len != sizeof(hash)) {
                BOOT_LOG_ERR("SHA256 TLV length invalid area=%u len=%u expected=%u",
                             flash_area_get_id(fap), len, (unsigned int)sizeof(hash));
                rc = -1;
                goto out;
            }
            rc = LOAD_IMAGE_DATA(hdr, fap, off, buf, sizeof(hash));
            if (rc) {
                BOOT_LOG_ERR("SHA256 TLV read failed area=%u off=%lu rc=%d",
                             flash_area_get_id(fap), (unsigned long)off, rc);
                goto out;
            }

            FIH_CALL(boot_fih_memequal, fih_rc, hash, buf, sizeof(hash));
            if (FIH_NOT_EQ(fih_rc, FIH_SUCCESS)) {
                BOOT_LOG_ERR("SHA256 mismatch area=%u", flash_area_get_id(fap));
                FIH_SET(fih_rc, FIH_FAILURE);
                goto out;
            }

            BOOT_LOG_INF("SHA256 verified area=%u", flash_area_get_id(fap));
            image_hash_valid = 1;
            break;
        }
        ...
        case EXPECTED_KEY_TLV:
        {
            ...
            if (key_id < 0) {
                BOOT_LOG_ERR("KEYHASH did not match a trusted public key area=%u",
                             flash_area_get_id(fap));
            } else {
                BOOT_LOG_INF("KEYHASH matched trusted key_id=%d area=%u",
                             key_id, flash_area_get_id(fap));
            }
#else
        ...
        case EXPECTED_SIG_TLV:
        {
            ...
            if (!EXPECTED_SIG_LEN(len) || len > sizeof(buf)) {
                BOOT_LOG_ERR("ECDSA signature TLV length invalid area=%u len=%u",
                             flash_area_get_id(fap), len);
                rc = -1;
                goto out;
            }
            rc = LOAD_IMAGE_DATA(hdr, fap, off, buf, len);
            if (rc) {
                BOOT_LOG_ERR("ECDSA signature TLV read failed area=%u off=%lu rc=%d",
                             flash_area_get_id(fap), (unsigned long)off, rc);
                goto out;
            }
            ...
            if (FIH_EQ(valid_signature, FIH_SUCCESS)) {
                BOOT_LOG_INF("ECDSA-P256 signature verified area=%u",
                             flash_area_get_id(fap));
            } else {
                BOOT_LOG_ERR("ECDSA-P256 signature verification failed area=%u",
                             flash_area_get_id(fap));
            }
#else
    ...
    rc = !image_hash_valid;
    if (rc) {
        BOOT_LOG_ERR("required SHA256 TLV missing or not validated area=%u",
                     flash_area_get_id(fap));
        goto out;
    }
...
out:
    if (rc) {
        BOOT_LOG_ERR("image validation failed area=%u rc=%d",
                     flash_area_get_id(fap), rc);
        FIH_SET(fih_rc, FIH_FAILURE);
    } else if (FIH_EQ(fih_rc, FIH_SUCCESS)) {
        BOOT_LOG_INF("image validation complete area=%u result=SUCCESS",
                     flash_area_get_id(fap));
    } else {
        BOOT_LOG_ERR("image validation complete area=%u result=FAILURE",
                     flash_area_get_id(fap));
    }
