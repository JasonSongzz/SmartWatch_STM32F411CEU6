                    ...
                    uint8_t key_id)
{
    ...
    if (rc) {
        BOOT_LOG_ERR("ECDSA public key DER parse failed key_id=%u rc=%d",
                     (unsigned int)key_id, rc);
        goto out;
    }
    BOOT_LOG_DBG("ECDSA public key parsed key_id=%u", (unsigned int)key_id);

    rc = bootutil_ecdsa_verify(&ctx, pubkey, end-pubkey, hash, hlen, sig, slen);
    if (rc != 0) {
        BOOT_LOG_ERR("ECDSA verification backend failed key_id=%u rc=%d sig_len=%lu",
                     (unsigned int)key_id, rc, (unsigned long)slen);
    } else {
        BOOT_LOG_DBG("ECDSA verification backend success key_id=%u",
                     (unsigned int)key_id);
    }
